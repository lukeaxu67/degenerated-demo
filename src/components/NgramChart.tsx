...
export default function NgramChart({ tokens, n }: Props) {
  ...
  const overlapRate = totalNgrams > 0 ? (repeatedNgrams / totalNgrams) : 0;
  ...
  return (
    <div>
      <div style={{ marginBottom: 8 }}>
        本轮共生成 {totalNgrams} 个 {n}-gram，其中 {repeatedNgrams} 个模式出现了两次及以上，对应的复读率为 {overlapRate.toFixed(3)}。矩形树图中的矩形越大，表示该 {n}-gram 在回复中的重复越严重。
      </div>
