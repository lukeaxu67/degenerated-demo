    ...
    >
      ...
      <Card title="方法总览（阅读指引）" style={{ marginBottom: 16 }}>
        ...
        <Paragraph>
          ...
          <Text code>X</Text> 的取值，经验概率为
          <Text code>p_i = count(x_i) / N</Text>。信息熵定义为：
          <Text code>H(X) = - \sum_i p_i \log_2 p_i</Text>。
          最小值为 0（所有概率集中在一个 token 上），最大值为
          <Text code>H_max = \log_2 |V|</Text>（所有 token 等概率出现，|V| 为去重后的 token
          个数）。我们使用
          <Text code>H_norm = H / H_max</Text> 进行标准化，使其位于 [0,1] 区间，
          再以 <Text code>1 - H_norm</Text> 作为「低多样性」退化特征。
        </Paragraph>
        <Paragraph>
          2）<Text strong>{ngramSize}-gram 重复率</Text>：
          仅考虑长度为 {ngramSize} 的固定窗口（例如「作为 / 一个 / AI」），统计所有
          {ngramSize}-gram 中有多少个模式在同一轮中出现了两次及以上。
        </Paragraph>
        <Paragraph>
          3）<Text strong>跨轮重叠率</Text>：
          对连续两轮助手回复，计算去重后的 token 集合交并比
          <Text code>J(A, B) = |A \cap B| / |A \cup B|</Text>，并用柱状图展示每个重复 token
          在前后两轮中的出现频次。
        </Paragraph>
        <Paragraph>
          4）<Text strong>拒答退化（模板化拒绝回答）</Text>：
          预置一组典型的拒答短语（例如「作为一个 AI 语言模型…」「我不能提供…」），只要回复中命中任意一条，就认为存在拒答退化倾向。
        </Paragraph>
        <Paragraph type="secondary">
          注意：所有统计量在内部都基于「去除句读标点后的 token 序列」计算
          （例如去掉中文逗号、句号、问号等），以避免标点本身对熵与重复指标的干扰；
          但界面展示的分词结果仍保留原始标点，以便对齐原文。
        </Paragraph>
      </Card>
                    ...
                    >
                      ...
                      <Card title="分词与预处理">
                        <Paragraph>
                          ...
                          tokenizer。首先在原文上获得 token 序列，然后再从 token 中剥离句号、逗号、问号等句读标点，只保留真正承载语义的信息 token。
                        </Paragraph>
                        <Paragraph type="secondary">
                          下方高亮的是「原始 token」（包含标点），用于精确对齐模型输出；后续所有指标与图表均基于「去标点后的 token 序列」计算。
                        </Paragraph>
                        <TokenHighlighter
                          text={turn.content}
                          tokens={turn.tokens}
                      ...
                      <Card title="信息熵分析（多样性）">
                        <Paragraph>
                          我们把每个 token 看作随机变量 X 的取值，估计其经验概率{" "}
                          <Text code>p_i = count(x_i) / N</Text>，其中 N 为本轮 token
                          总数。信息熵定义为：
                        </Paragraph>
                        <Paragraph>
                          <Text code>
                            H(X) = - \sum_{"{"}i=1{"}"}^{"{"}V{"}"} p_i \log_2 p_i
                          </Text>
                        </Paragraph>
                        <Paragraph>
                          当所有概率集中在一个 token 上时，H(X) = 0；当所有 token
                          等概率出现时，H(X) 取得最大值{" "}
                          <Text code>H_max = \log_2 V</Text>，其中 V 为词表大小
                          |V|。为了便于不同回复之间比较，我们使用归一化熵{" "}
                          <Text code>H_norm = H / H_max</Text>，并将{" "}
                          <Text code>1 - H_norm</Text> 视为「低多样性」退化特征。
                        </Paragraph>
                        <Paragraph>
                          对本轮回复，去除句读标点后共有{" "}
                          ...
                          <Text code>
                            H(X) ≈ {turn.entropy.toFixed(3)} bit
                          </Text>
                          ，最大熵{" "}
                          <Text code>
                      ...
                      <Card title={`${ngramSize}-gram 重复率（局部模式复用）`}>
                        ...
                        <Paragraph type="secondary">
                          简单说，就是回答中是否不断复用相同的局部短语模式，比如「作为 / 一个 / AI」「我 /
                          不能 / 提供」等。
                        </Paragraph>
                      ...
                      <Card title="跨轮重叠率（与上一轮的复读情况）">
                        <Paragraph>
                          对连续两轮助手回复，我们计算去重后的 token 集合的 Jaccard
                          指数：
                          <Text code>
                            J(A, B) = |A \cap B| / |A \cup B|
                          </Text>
                          。它刻画的是两轮回复在用词层面上的复用程度。
                        </Paragraph>
                        <OverlapChart
                      ...
                      <Card title="综合退化评分分解">
                        ...
                        <Paragraph type="secondary">
                          这个分数并不是一个「绝对正确」的退化判定，而是把多种退化现象（重复、复读、拒答模板、低熵）压缩到一个可比较的尺度上，方便在不同对话样本之间做横向分析。
                        </Paragraph>
                      </Card>
                    </Space>
                  ),
